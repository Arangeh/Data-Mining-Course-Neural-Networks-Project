...
def find_frequent_one(data_set, support):
	...
	frequent_1 = []
	for key in sorted(candidate_one.keys()):
		# check if given item has sufficient count.
		cnt = candidate_one[key]
...
class HTree:
	...
	def recur_insert(self, node, itemset, index, cnt):
		...
		redistributes itemsets among children converting itself into intermediate node.
		:param node:
		:param itemset:
		:param index:
		:return:
			...
			if len(node.bucket) == self.max_leaf_cnt:
				...
				for old_itemset, old_cnt in node.bucket.items():
					...
					if hash_key not in node.children:
						node.children[hash_key] = HNode.HNode()
	...
	def insert(self, itemset):
		...
		# which can be easily hashed in leaf node bucket
	...
	def hash(self, val):
		val_eqv = ord(val)
		return val_eqv % self.max_child_cnt
...
def apriori_generate_frequent_itemsets(dataset, support):
	...
	prev_frequent = [x[0] for x in all_frequent_itemsets]#x is of the form ([itemset],count)
	#so prev_frequent is a list containing all of the itemset without their counts
	
	length = 2#we always aim to generate k-subsets
	...
	for itemset in f_itemsets:
		...
		for i in range(1, length):
			...
			for left in lefts:
				...
				# conf = union_support / count_all_occurances(hash_map, tuple(left))
				if conf >= confidence:
...
def print_rules(rules):
	for item in rules:
		...
		conf = item[2]
		print(' ==> '.join([left, right]) + " : " + str(conf))
	print('Total Rules Generated: ', len(rules))

#the following method returns total number of occurances of a tuple t in transaction Database d(Dictionary type)
# in order to do so, we check the keys of d whose t is a subset of.
# def count_all_occurances(d, t):
# 	cnt = 0
#
# 	# print(d)
# 	for record in d.keys():
# 		if(set(t).issubset(set(record))):
# 			cnt += d[record]
#
# 	return cnt
